...
Current Phase: Post-Phase guest access DONE — Phases 0–9 verified; secure read-only guest access + suggestion inbox is deployed to production
Current Task: PRODUCTION RELEASE MANUAL CHECK — guest/admin code system is deployed and automated gates pass; one physical Authenticator QR/code UX check remains; Phase 10 product work not started
...
| 1 Schema/security | Provider-neutral forward schema, RLS/AAL2, share/privacy, quota | DONE | supabase/migrations, tests/security/database.test.ts | 26 local PostgreSQL tests PASS; 11 migrations live; two-user live RLS/share audit PASS | Historical migrations preserved; `202609210001_drink_category.sql` adds `drink` forward-only without rewriting existing `food` rows |
| 2 Auth/MFA | Passwordless email identifier + TOTP enrollment/login, AAL2 guards and refresh-safe private UI | DONE | `src/components/auth/LoginForm.tsx`, `src/features/auth/AuthProvider.tsx`, `src/services/totpAuth.ts`, `supabase/functions/auth-totp` | Final suite 99 frontend + 69 security tests PASS; Playwright 14/14; same-user `TOKEN_REFRESHED` keeps ready UI/cache while downgrade still closes access | Private custom-place drafts survive same-user refresh/reload but are cleared on sign-out or account switch; one physical Authenticator UX check remains manual |
...
| 4 Geoapify Places | Provider abstraction, Edge Function, search/browse/details/reverse, durable CRUD, private place, attribution, category UX and Google Maps link-to-location resolution | DONE | `src/services/geoProvider.ts`, `src/components/places`, `src/utils/customPlaceDraft.ts`, `supabase/functions/geo`, `supabase/functions/_shared/googleMaps.ts` | Final suite 99 frontend + 69 security tests PASS; Playwright 14/14; live `drink` persists through AAL2 RLS and renders as `🥤 Uống`; deployed Edge resolves Google Maps links | `food` now displays as `Ăn`, `drink` is first-class `Uống`, `cafe` stays separate; new `Khác` drafts require a specific type name; custom draft restores after reload |
...
| 5 Planner UI | Start/candidates, transport, party/time/preferences, planner state | DONE | src/components/planner/PlannerPanel.tsx, src/stores/plannerStore.ts, src/hooks/usePlaces.ts | Final gate 101 frontend + 69 security tests PASS; Playwright 16/16; Planner desktop/mobile responsive overflow assertions PASS | Planner form now responds to its own container width; narrow desktop panels/iPhone stack two-column controls and constrain datetime inputs |
...
| Post-phase Guest access | `Try another way` access-code login, read-only `/guest`, map/list view, suggestion inbox, AAL2 admin code/suggestion management | DONE | `src/services/guestAccess.ts`, `src/pages/GuestPage.tsx`, `src/pages/AccountPage.tsx`, `supabase/functions/guest-access`, migrations `202609210002` + `202609210003` | `npm run check` PASS = 101 frontend + 76 security; Playwright 20/20; CI run `35592613137` SUCCESS; remote migrations 13/13; `guest-access` ACTIVE; production CORS 204 and invalid-code boundary 401 | Guest has no Supabase Auth session and no direct table grants; plaintext codes/tokens are not stored; 12h guest capability, server-side HMAC + rate-limit attempts; valid production code journey still depends on the existing physical-admin TOTP/manual acceptance path |
| 10 External handoffs | Tasks beyond current review/booking handoffs | TODO | — | Not started | Outside Prompt 2 |
| 11 PWA/Lucky Wheel/Polish | Full phase | TODO | `public/` contains production favicon/Apple Touch/PWA/OG assets; saved-place TXT export and Planner responsive polish landed independently | Export/branding/responsive subset verified in final 101/69 + Playwright 16/16 gate | Full PWA offline/service-worker work and Lucky Wheel remain unstarted; do not mark Phase 11 DONE |
...
- RETAIN: validated external Google Maps URLs only. Custom-place input accepts allowlisted HTTPS Google Maps/share links; Edge extracts coordinates directly or follows bounded Google-only redirects and may fall back to Geoapify text geocoding. No Google Maps Platform API/SDK/key is used.
...
- Place taxonomy keeps existing `food` rows but displays them as `Ăn`; new enum value `drink` is `Uống`; `cafe` stays separate. New `Khác` drafts require `sub_category`, while existing saved rows remain readable.
- Same-user auth refresh events keep private UI mounted and revalidate AAL2 in the background. Custom-place drafts persist in per-tab `sessionStorage` across reloads, but are cleared on sign-out or account switch to avoid cross-user leakage.
...
- Saved-place export is client-side TXT download with two modes: all owner places (fetched across pages) or an explicit checkbox selection. `start_point` rows are numbered `H0`, `H1`, ...; other destinations are numbered `D1`, `D2`, ...; saved Google Maps URLs are preserved and coordinate/search links are generated only when needed.
- Production branding assets live under `public/`: browser favicon, 180px Apple Touch Icon, 192/512 manifest icons and a 1200x630 OG image. This does not imply full offline PWA/service-worker support.
- Planner responsive layout uses its own container width rather than viewport width; narrow desktop sidebars and mobile layouts stack paired controls and constrain native datetime inputs to avoid horizontal overflow.
...
- 2026-09-21 Google Maps link-to-location final gate PASS: `npm run check` = 94/94 frontend + 67/67 security, build/audit PASS; Playwright 14/14 desktop/mobile; `npm audit --omit=dev` = 0 vulnerabilities. Security coverage includes long-link coordinate extraction, `!3d/!4d`, query coordinates, text fallback, bounded short-link redirects and redirect escape rejection.
- 2026-09-21 Google Maps resolver live PASS after `geo` redeploy: authenticated production Edge resolved both a coordinate-bearing Google Maps long URL and real `maps.app.goo.gl` short URL to finite coordinates; temporary Auth user cleanup PASS.
- 2026-09-21 deployed bundle smoke PASS for commit `685861b`: GitHub Actions CI SUCCESS; Vercel production Ready; production Dashboard chunk contains `Liên kết Google Maps`, `Xác định vị trí` and `Đã xác định vị trí`, while `Vĩ độ` and `Kinh độ` are absent.
- 2026-09-21 Google Maps pin-priority bugfix PASS: parser now prefers `!3d<lat>!4d<lng>` place-pin coordinates, then explicit query coordinate pairs, with `@<lat>,<lng>` only as camera-center fallback. Regression uses the supplied Zapí pizza URL containing both coordinate sets and expects `10.841487, 106.6778395` instead of camera center `10.8419401, 106.6755569`. Final gate PASS: 94/94 frontend, 68/68 security, Playwright 14/14, build/audit PASS, `npm audit --omit=dev` = 0 vulnerabilities. Deployed `geo` live acceptance with the same Zapí URL returned exactly `10.841487, 106.6778395`; temporary Auth user cleanup PASS. Commit `1740f0e` GitHub CI SUCCESS and Vercel production Ready.
- 2026-09-21 TXT export + branding + Planner responsive polish PASS: `npm run check` = 101/101 frontend + 69/69 security, production build/audit PASS; Playwright 16/16 desktop/mobile; `npm audit --omit=dev` = 0 vulnerabilities. E2E reads downloaded TXT files and verifies both all-place and selected-only exports. Planner E2E asserts no horizontal overflow and that both datetime controls remain inside the Planner shell. Commit `8a037b9` GitHub Actions run `35584143755` SUCCESS; Vercel canonical alias points to production deployment `smart-food-route-r9pzcch99-trunknguen.vercel.app` Ready. Production smoke: root/favicons/Apple Touch Icon/manifest/OG image HTTP 200, deployed Dashboard bundle contains `Xuất tất cả TXT` + `Xuất đã chọn`, and deployed CSS contains `@container planner-shell`.
- 2026-09-21 guest access final gate PASS: `npm run check` = 101/101 frontend + 76/76 security, build/runtime audit PASS; Playwright 20/20 desktop/mobile; `npm audit --omit=dev` = 0 vulnerabilities. Remote migration history is 13/13 through `202609210003`; `guest-access` Edge Function version 1 is ACTIVE with `verify_jwt=false`, production CORS preflight returns 204 and a valid-format unknown code returns 401 after explicit service-role grants. Commit `24a9351` GitHub Actions run `35592613137` SUCCESS; Vercel production deployment `smart-food-route-kg70f3388-trunknguen.vercel.app` is Ready. Production bundle smoke confirms `Try another way`, `/guest`, read-only guest copy, suggestion UI and Account `Access codes`/suggestion management are deployed.

## Current Repository State

- React application implements Phases 0–9 under v3 plus the post-phase secure guest-access feature: `Try another way` code exchange, read-only `/guest`, map/list browsing, guest suggestions, and AAL2-only admin code/suggestion management.
- Saved-place UI now supports two verified TXT exports: all saved owner places or an explicit checkbox selection. Browser/iPhone branding assets and manifest metadata are deployed; Planner responsive controls no longer overflow narrow desktop/iPhone containers.
- Supabase remote contains 13 migrations plus active `geo`, `auth-totp` and `guest-access` Edge Functions. `auth-totp` provides the passwordless email→TOTP bootstrap while returning only AAL2 browser sessions after valid TOTP; `geo` provides Places + Google Maps link-to-location resolution + Route Matrix + Routing; `guest-access` mediates guest capabilities and admin actions through server-side secrets/service-role access. `ALLOWED_ORIGINS` includes local development and `https://smart-food-route.vercel.app`; provider/admin secrets remain server-only.
...
- No active Google Maps Platform dependency, API request, key or Map ID exists. Google Maps is used only as an allowlisted external link/redirect source for user-supplied location URLs; coordinates are stored internally after resolution. Compatibility field names occur only in historical migrations where required.
...
Completed: Phase 0–9 v3 implementation plus private GitHub/Vercel production deployment, passwordless email + TOTP auth, Google Maps link-to-location UX, split Eat/Drink categories, refresh-safe custom-place drafts, saved-place TXT export, deployed browser/iPhone brand assets, Planner narrow-container fixes, and secure guest access. Guest flow is `Try another way` → access code → read-only `/guest`; guests can browse saved places/map and submit suggestions, while only the configured AAL2 admin can manage codes/suggestions. Production `auth-totp`, `geo`, and `guest-access` are live.
Production verification: canonical alias is Ready; guest-access migrations are remote through `202609210003`; Edge function `guest-access` is ACTIVE; production CORS preflight returns 204; valid-format unknown guest code returns 401; deployed bundles contain `Try another way`, guest read-only/suggestion UI and Account `Access codes` management. Latest local gate = 101 frontend + 76 security tests, Playwright 20/20 desktop/mobile, build/audit PASS, npm audit 0 vulnerabilities; GitHub Actions run `35592613137` is SUCCESS for commit `24a9351`.
Remaining deployment/manual work: one physical Authenticator QR scan/code UX check; because admin guest-code creation is intentionally AAL2-gated, a full valid production guest-code acceptance depends on that physical admin path.
