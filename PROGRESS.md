...
- Supabase remote contains all eleven migrations plus active `geo` and `auth-totp` Edge Functions. `auth-totp` provides the passwordless email→TOTP bootstrap while returning only AAL2 browser sessions after valid TOTP; `geo` provides Places + Google Maps link-to-location resolution + Route Matrix + Routing. `ALLOWED_ORIGINS` includes local development and `https://smart-food-route.vercel.app`; `GEOAPIFY_API_KEY` remains Edge-only.
...
Completed: Phase 0–9 v3 implementation plus private GitHub/Vercel production deployment, passwordless email + TOTP auth, Google Maps link-to-location UX, split Eat/Drink categories, refresh-safe custom-place drafts, saved-place TXT export, deployed browser/iPhone brand assets, and Planner narrow-container overflow fixes. Production `auth-totp` and `geo` are live; global `@trunk.ng` links to the requested Instagram profile.
Production verification: canonical alias is Ready; `/login` and brand assets return HTTP 200; deployed Dashboard bundle contains both TXT export modes; deployed CSS contains the Planner container-query fix. Latest local/CI gate = 101 frontend + 69 security tests, Playwright 16/16 desktop/mobile, build/audit PASS, npm audit 0 vulnerabilities.
