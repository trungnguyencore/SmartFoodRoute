...
| 4 Geoapify Places | Provider abstraction, Edge Function, search/browse/details/reverse, durable CRUD, private place, attribution and Google Maps link-to-location resolution | DONE | `src/services/geoProvider.ts`, `src/components/places`, `supabase/functions/geo`, `supabase/functions/_shared/googleMaps.ts` | 94 frontend + 67 security tests PASS; Playwright 14/14; deployed Edge resolves both long and short Google Maps URLs live | Custom-place UI no longer asks users for latitude/longitude; coordinates remain internal for Map/Planner |
