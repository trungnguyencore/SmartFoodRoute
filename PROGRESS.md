...
| 4 Geoapify Places | Provider abstraction, Edge Function, search/browse/details/reverse, durable CRUD, private place, attribution and Google Maps link-to-location resolution | DONE | `src/services/geoProvider.ts`, `src/components/places`, `supabase/functions/geo`, `supabase/functions/_shared/googleMaps.ts` | 94 frontend + 68 security tests PASS; Playwright 14/14; deployed Edge resolves long/short Google Maps URLs and prioritizes place-pin coordinates over camera center | Custom-place UI no longer asks users for latitude/longitude; coordinates remain internal for Map/Planner |
...
- Supabase remote contains all ten migrations plus active `geo` and `auth-totp` Edge Functions. `auth-totp` provides the passwordless email→TOTP bootstrap while returning only AAL2 browser sessions after valid TOTP; `geo` provides Places + Google Maps link-to-location resolution + Route Matrix + Routing. `ALLOWED_ORIGINS` includes local development and `https://smart-food-route.vercel.app`; `GEOAPIFY_API_KEY` remains Edge-only.
...
Completed: Phase 0–9 v3 implementation plus private GitHub/Vercel production deployment, passwordless email + TOTP auth, and Google Maps link-to-location UX for custom places. Production `auth-totp` and `geo` are live; the deployed frontend shows no password field, custom-place forms no longer expose latitude/longitude inputs, and global `@trunk.ng` links to the requested Instagram profile.
